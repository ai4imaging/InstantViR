from instantvir.models import get_diffusion_wrapper, get_text_encoder_wrapper, get_vae_wrapper
...
class ODERegression(nn.Module):
    def __init__(self, args, device):
        ...
        See Sec 4.3 of InstantViR https://arxiv.org/abs/2412.07772 for details
        """
        super().__init__()
        ...
        # Move generator to target device (keep weights in their original dtype, typically float32)
        ...
        self.args = args
        self.device = device
        # For stability, keep generator/text encoder in float32; mixed_precision only controls data casting
    ...
    def _process_timestep(self, timestep):
        ...
        if self.args.generator_task == "image":
            ...
            if timestep.shape[1] < self.num_frame_per_block:
                # Not enough frames to form a block (e.g., T=1). Skip block broadcasting.
                return timestep
