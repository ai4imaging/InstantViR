from instantvir.models.wan.wan_base.modules.attention import attention
from instantvir.models.wan.wan_base.modules.model import (
    WanRMSNorm,
    rope_apply,
    WanLayerNorm,
...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, block_mask, kv_cache=None, current_start=0, current_end=0):
        ...
        if kv_cache is None:
            ...
            # Use FlashAttention-based implementation to avoid TorchInductor Triton autotuning issues
            # seen with torch.nn.attention.flex_attention on some environments.
            x = attention(
                q=roped_query,
                k=roped_key,
                v=v,
                ...
                window_size=(-1, -1),
                deterministic=False,
                # Force flash attention to run in half precision to satisfy kernel requirement
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _forward_inference(
        ...
        See Algorithm 2 of InstantViR paper https://arxiv.org/abs/2412.07772 for details.
    ...
    def _forward_train(
        ...
        x = [u.flatten(2).transpose(1, 2) for u in x]
        seq_lens = torch.tensor([u.size(1) for u in x], dtype=torch.long)
        # Ensure seq_len is an exact multiple of num_frames * frame_seqlen to avoid unflatten mismatch
        num_frames = int(grid_sizes[0, 0].item())
        frame_seqlen = int((grid_sizes[0, 1] * grid_sizes[0, 2]).item())
        desired_len = num_frames * frame_seqlen
        if seq_len < desired_len:
            # safety: never pad beyond provided seq_len, but prefer exact multiple
            seq_len_eff = seq_len - (seq_len % num_frames)
        else:
            seq_len_eff = desired_len
        assert seq_lens.max() <= seq_len_eff
        x = torch.cat([
            torch.cat([u, u.new_zeros(1, seq_len_eff - u.size(1), u.size(2))],
                      dim=1) for u in x
        ])
